...
export class TaskManager {
    constructor(employeeId) {
        ...
        this.openModalTransferBtnMobile = document.getElementById('openModalTransferBtnMobile');
        this.closeModalTransferBtn = document.getElementById('closeModalTransferBtn');
        this.openModalCreateBtnMobile = document.getElementById('openModalCreateBtnMobile');

        this.basePath = window.location.pathname.split('/tasks')[0];
        this.eventsource = null;
        this.employeeId = employeeId;
        this.notask = document.getElementById('no_tasks');
        // Status mapping
        this.statusMap = {
            'todo': 1,
            'in_progress': 2,
    ...
    initializeEventListeners() {
        ...
        this.openModalCreateBtnMobile?.addEventListener('click', () => this.openModal('create'));
        ...
        this.openModalTransferBtnMobile?.addEventListener('click', () => this.openModal('transfer'));
    ...
    async updateTaskStatus(projectID, taskId, taskStatusId) {
        ...
        try {
            const response = await fetch(`${this.basePath}/tasks/${taskId}/update-status`, {
                method: 'POST',
                headers: {
    ...
    async fetchTasks(date = '') {
        ...
        this.isLoading = true;
        this.toggleLoading
        await fetch(`${this.basePath}/tasks/get-tasks?date=${date}`)
    ...
    async populateEmployeeDropdown(projectId) {
        try {
            ...
            const activeEmployees = project.employees.filter(employee => employee.isformeremployee == false);

            employeeDropdown.innerHTML = "";

            employeeDropdown.innerHTML = activeEmployees.map(employee => `
            <li class="block px-4 py-2 text-black hover:bg-[#C3C3C3] cursor-pointer rounded-md"
                onclick="taskManager.listOnClick(event, 'trasfer-employee-value', 'trasfer-employee-dropdown', 'trasfer-employee-icon', 'assigned_project_employee_id', ${employee.projectEmployeeid})">
                ${employee.user.name}
            </li>
        `).join('');
        } catch (error) {
    ...
    async listOnClick(event, dropdownValueId, dropdownId, iconId, inputId, data) {
        if (event.target.tagName === "LI") {
            ...
            if (document.getElementById('modalTransfer').contains(document.getElementById(inputId))) {
                ...
                try {
                    const project = this.projects.find(project => project.id == data);
                    const employee = project.employees.find(employee => employee.id == this.employeeId);
                    document.getElementById('employee_create').value = employee.projectEmployeeid;
                } catch (error) {
    ...
    renderProjectsDropdown() {
        ...
        const dropdownHTML = allProjects.map(project => `
            <li
                class="block px-4 py-2 text-black hover:bg-[#C3C3C3] cursor-pointer rounded-md w-full  sm:text-sm xs:text-[12px]"
    ...
    templateTasks(task, project, employee) {
        return `
                <div class="bg-white border border-[#7D7D7D] h-fit sm:px-3 xs:px-2 sm:py-4 xs:py-3 rounded-md mb-2"
                    ...
                    <div class="flex justify-between items-start">
                        <h4 class="font-semibold sm:text-sm xs:text-[12px] sm:pb-4">${task.name}</h4>
                        <p class="sm:text-[10px] xs:text-[8px] font-medium px-2 py-[2px] rounded-md text-white sm:inline xs:hidden"
                            style="background-color: ${task.task_level.color}">${task.task_level.name}</p>
                    </div>
                    <p class="sm:text-xs xs:text-[10px] sm:mb-3 xs:mb-2">${project.name}</p>
                    <div class="flex items-center justify-between">
                        <p class="sm:text-xs xs:text-[10px] text-gray-500 font-medium flex items-center">
                            <svg class="sm:size-4 xs:size-3 mr-1" viewBox="0 0 35 35" fill="currentColor"
                        ...
                        <p class="sm:text-xs xs:text-[11px] text-gray-500 sm:inline xs:hidden">${employee.user.name}</p>
                    </div>
                    <div class="sm:hidden xs:flex justify-between mt-4">
                        <p class="sm:text-[10px] xs:text-[8px] font-medium px-2 py-[2px] rounded-md text-white"
                            style="background-color: ${task.task_level.color}">${task.task_level.name}</p>
                        <p class="sm:text-xs xs:text-[11px] text-gray-500">${employee.user.name}</p>
                    </div>
                </div>
        `;
    }
    ...
    getTimelineData(task) {
        let timeline;
        const staus = typeof task.task_status_id === 'string' ? parseInt(task.task_status_id) : task.task_status_id;
    ...
    renderEditTask(task) {
        ...
        const content = `
                <form action="${this.basePath}/tasks/${task.id}" method="POST" enctype="multipart/form-data">
    ...
    setupReverb() {
        ...
        window.Echo.private(`new_task_${window.employeeId}`).listen('SendNotification', (data) => {
            this.appendTaskToDOM(data)
        });
    }

    closeStream() {
        if (window.Echo) {
            window.Echo.private(`new_task_${window.employeeId}`).stopListening('SendNotification');
            window.Echo.disconnect();
        }
    }

    async appendTaskToDOM(task) {
        ...
        await this.projects.find(async prj => {

            if (prj.id == task.task.project_id) {
                const employee = await prj.employees.find(emp => emp.id == task.employee_id);
                prj.tasks.push(task.task);

                const template = document.createElement('div');

                template.innerHTML = this.templateTasks(task.task, prj, employee);

                todo.appendChild(template)
            }
        });
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    window.taskManager = new TaskManager(employeeId);
});

window.addEventListener('beforeunload', () => {
    window.taskManager.closeStream();
})
