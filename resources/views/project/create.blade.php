<x-layouts.layout :title="$title" :active="$active">
    <main class="flex space-x-4">
        <form action="{{ route('projects.store') }}" method="POST" enctype="multipart/form-data" class="flex-1 flex" id="create_project">
            @csrf
            <div class="bg-white rounded shadow w-2/5 pt-4 px-5 pb-4 mr-4">
                ...
                <div class="space-y-4 mt-6">
                    <div class="flex flex-col space-y-2">
                        ...
                        <input type="text" name="name" id="name" class="w-full bg-primary-white border border-primary-white px-3 py-2 text-sm rounded focus:outline-none" placeholder="project name.." required value="{{old('name')}}">
                        @error('name')
                    ...
                    <div class="flex flex-col space-y-2">
                        ...
                        <div class="flex items-center justify-between w-full gap-4">
                            <div class="flex items-center w-[50%] gap-2">
                                ...
                                <input type="date" name="start_date" id="start_date" class="w-full bg-primary-white border border-primary-white px-3 py-2 text-sm rounded focus:outline-none" required value="{{old('start_date')}}">
                                @error('start_date')
                                    <span class="text-red-600 text-sm">{{ $message }}</span>
                                @enderror
                            </div>
                            <div class="flex items-center w-[50%] gap-1">
                                ...
                                <input type="date" name="end_date" id="end_date"
                                    class="w-full bg-primary-white px-3 py-2 text-sm rounded focus:outline-none border border-primary-white" required value="{{old('end_date')}}">
                                @error('end_date')
                                    <span class="text-red-600 text-sm">{{ $message }}</span>
                                @enderror
                            </div>
                    ...
                    <div class="flex flex-col space-y-2">
                        ...
                        <select name="project_level_id" class="w-full bg-primary-white border border-primary-white px-3 py-2 text-sm rounded focus:outline-none" id="level" required>
                            ...
                            @foreach ($levels as $level)
                                <option value="{{ $level->id }}" {{ old('project_level_id') == $level->id ? 'selected' : '' }}>{{ $level->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="flex flex-col space-y-2">
                        ...
                        <textarea name="description" id="description" rows="2" placeholder="project description.."class="w-full bg-primary-white border border-primary-white px-3 py-1 text-sm rounded focus:outline-none" value="{{old('description')}}"></textarea>
                        @error('description')
                            <span class="text-red-600 text-sm">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="flex flex-col space-y-2">
                        ...
                        <select name="director_id" class="w-full bg-primary-white border border-primary-white px-3 py-2 text-sm rounded focus:outline-none" id="director">
                            ...
                            @foreach ($employees->where('role.name', 'Project Director') as $employee)
                                <option value="{{ $employee->id }}" {{old('director_id') == $employee->id ? 'selected' : ''}}>{{ $employee->user->name }}</option>
                            @endforeach
                        </select>
                        @error('director_id')
                            <span class="text-red-600 text-sm">{{ $message }}</span>
                    ...
                    <div class="flex flex-col space-y-2">
                        ...
                        <select name="analyst_id"
                            ...
                            @foreach ($employees->where('role.name', 'Analyst') as $employee)
                                <option value="{{ $employee->id }}" {{old('analyst_id') == $employee->id ? 'selected' : ''}}>{{ $employee->user->name }}</option>
                            @endforeach
                        </select>
                        @error('analyst_id')
                            <span class="text-red-600 text-sm">{{ $message }}</span>
                                ...
                                <select name="designer_id" class="w-full bg-primary-white border border-primary-white px-3 py-2 text-sm rounded focus:outline-none" id="designer">
                                    <option value="">Select project designer</option>
                                        @foreach ($employees->where('role.name', 'Designer') as $employee)
                                            <option value="{{ $employee->id }}" {{old('designer_id') == $employee->id ? 'selected' : ''}}>{{ $employee->user->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('designer_id')
                                        <span class="text-red-600 text-sm">{{ $message }}</span>
                            ...
                            <div class="w-[50%] space-y-2">
                                ...
                                <select name="engineer_web_id" class="w-full bg-primary-white border border-primary-white px-3 py-2 text-sm rounded focus:outline-none" id="engineer_web">
                                    ...
                                    @foreach ($employees->where('role.name', 'Engineer Web') as $employee)
                                        <option value="{{ $employee->id }}" {{old('engineer_web_id') == $employee->id ? 'selected' : ''}}>{{ $employee->user->name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="w-[50%] space-y-2">
                                ...
                                <select name="engineer_mobile_id"class="w-full bg-primary-white border border-primary-white px-3 py-2 text-sm rounded focus:outline-none" id="engineer_mobile">
                                    ...
                                    @foreach ($employees->where('role.name', 'Engineer Mobile') as $employee)
                                        <option value="{{ $employee->id }}" {{old('engineer_mobile_id') == $employee->id ? 'selected' : ''}}>{{ $employee->user->name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="space-y-2 w-[50%]">
                                ...
                                <select name="engineer_tester_id" class="w-full bg-primary-white border border-primary-white px-3 py-2 text-sm rounded focus:outline-none" id="engineer_tester">
                                    ...
                                    @foreach ($employees->where('role.name', 'Engineer Tester') as $employee)
                                        <option value="{{ $employee->id }}" {{old('engineer_tester_id') == $employee->id ? 'selected' : ''}}>{{ $employee->user->name }}</option>
                                    @endforeach
                                </select>
            ...
            <div class="fixed bottom-4 right-4 flex space-x-4">
                ...
                <button type="submit" class="bg-black text-white px-4 py-2 rounded-md">Create</button>
            </div>
        </form>
    </main>
