<x-layouts.layout :title="$title" :active="$active">
    <main class="max-w-screen w-full mx-auto rounded-xl bg-white border">
        ...
        <div class="flex items-center justify-between mb-2 pt-3 px-6">
            <form action="{{ route('admin.index') }}" method="GET" class="sm:w-1/2 xs:w-[60%] flex" id="filterForm">
                <div class="flex items-center sm:w-[60%] xs:w-[50%]">
                    <input type="text" placeholder="Search SDM"
                        class="border rounded-lg w-full pl-9 pr-4 sm:py-1 xs:py-2 focus:outline-none sm:text-sm xs:text-[10px]"
                        value="{{ request('search') }}" name="search" />
                    <svg class="sm:size-5 xs:size-4 ml-3 z-0 absolute" viewBox="0 0 41 41" fill="none"
                ...
                <div class="sm:ml-5 xs:ml-2 sm:w-[40%] xs:w-[50%]">
                    <select name="role"
                        class="w-[94%] primary-gray font-medium rounded-lg sm:py-1 xs:py-2 px-2 sm:text-sm xs:text-[10px] border outline-none"
                        id="roleFilter">
            ...
            <a href="{{ route('admin.create') }}"
                class="bg-primary-black flex justify-center items-center hover:bg-zinc-500 cursor-pointer text-white sm:text-xs xs:text-[10px] px-4 sm:py-1 xs:py-2 rounded-lg hover:bg-gray-800">
        ...
        <div class="sm:overflow-x-auto">
            <table class="w-full border-collapse xs:table-fixed sm:table-auto">
                <thead class="bg-[#F9F9F9]">
                    <tr>
                        <th class="w-[3%] pl-4 py-2 primary-gray text-left font-medium sm:text-sm xs:text-[11px]">No
                        </th>
                        <th
                            class="sm:w-[10%] xs:w-[6%] px-4 py-2 primary-gray text-left font-medium sm:text-sm xs:text-[11px]">
                            Username</th>
                        <th
                            class="sm:w-[8%] xs:w-[4%] px-4 py-2 primary-gray text-left font-medium sm:text-sm xs:text-[11px]">
                            Divisi</th>
                        <th class="w-[3%] px-2 py-2 primary-gray text-left font-medium text-sm hidden sm:table-cell">
                            Role</th>
                        <th
                            class="sm:w-[8%] xs:w-[6%] px-4 py-2 primary-gray text-left font-medium sm:text-sm xs:text-[11px]">
                            Email</th>
                        <th
                            class="sm:w-[6%] xs:w-[4%] py-2 primary-gray text-left font-medium sm:text-sm xs:text-[11px]">
                            Action</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-200">
                    ...
                    @foreach ($employees as $index => $employee)
                        <tr class="group hover:bg-[#F5F5F5] cursor-pointer font-medium"
                            onclick="window.location='{{ route('admin.edit', $employee->id) }}'">
                            <td class="pl-6 py-2 sm:text-sm xs:text-[11px]">
                                {{ $employees->firstItem() + $index }}
                            </td>
                            <td class="px-4 py-2 sm:text-sm xs:text-[11px]">
                                {{ $employee->user->name ?? 'No username' }}
                            </td>
                            <td class="px-2 py-2 sm:text-sm xs:text-[11px]">
                                {{ $employee->role->name ?? 'No devisi' }}
                            </td>
                            <td class="px-4 py-2 text-sm hidden sm:table-cell">
                                {{ $employee->user->role ?? 'No role' }}
                            </td>
                            <td class="px-4 py-2 sm:text-sm xs:text-[11px] break-words whitespace-normal">
                                {{ $employee->user->email ?? 'No email' }}
                            </td>
                            <td cclass="py-2 sm:text-sm xs:text-[11px]">
                                <button type="button"
                                    class="bg-primary-red text-white px-2 py-[3px] sm:text-sm xs:text-[11px] rounded-lg delete-btn-user"
                                    data-id="{{ $employee->id }}">
                                    Delete
                                </button>
                            </td>
