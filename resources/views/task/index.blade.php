<x-layouts.layout :title="$title" :active="$active">
  <main class="p-2">
      <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between w-full pb-2 mb-3 gap-4">
          <!-- Filter Section -->
          <div class="flex flex-col sm:flex-row w-full lg:w-[40%] gap-3">
              <div class="w-full sm:w-fit relative z-20">
                  <button id="dropdown-button-projects" 
                      class="inline-flex items-center w-full px-2 py-1 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm z-20" 
                  ...
                  <ul id="projects-dropdown"
                      class="w-full sm:w-fit absolute right-0 left-0 mt-2 rounded-md shadow-lg bg-white p-1 space-y-1 transform opacity-0 scale-95 -translate-y-2 hidden transition-all duration-300 ease-out origin-top">
                  </ul>
              </div>
          ...
          <!-- Action Buttons -->
          <div class="w-full lg:w-[30%] flex flex-col sm:flex-row justify-end gap-2 sm:space-x-4 sm:gap-0">
              {{-- Create Task Button - Hidden jika tidak ada permission create --}}
              @if ($canCreate)
              ...
              {{-- Transfer Task Button - Hanya untuk Admin dan Project Director --}}
              @if ($canTransfer)
                  <div class="w-full sm:w-1/3">
                      <button id="openModalTransferBtn"
                          ...
                          <svg class="size-3 text-white mr-1" viewBox="0 0 18 18" fill="none"
                              ...
                              <path
                                  d="M17.4117 2.44334C17.8288 1.28955 16.7108 0.171484 15.557 0.589552L1.45567 5.68939C0.298018 6.10843 0.158019 7.68801 1.22298 8.30497L5.7242 10.9109L9.74364 6.89146C9.92573 6.71558 10.1696 6.61826 10.4228 6.62046C10.6759 6.62266 10.9181 6.72421 10.0971 6.90322C11.2761 7.08223 11.3777 7.3244 11.3799 7.57755C11.3821 7.83071 11.2847 8.0746 11.1089 8.25669L7.08944 12.2761L9.69633 16.7773C10.3123 17.8423 11.8919 17.7013 12.3109 16.5447L17.4117 2.44334Z"
                                  fill="white" />
                          </svg>
                          Transfer task
                      </button>
              ...
              {{-- See All Task Button - Hanya untuk Admin dan Project Director --}}
              @if ($canSeeAllTask)
                  <div class="w-full sm:w-1/3">
      ...
      <div class="container mx-auto max-w-full">
          <div class="flex flex-col lg:flex-row lg:space-x-4 w-full space-y-4 lg:space-y-0" id="kanban-board">
              <!-- To Do Column -->
              <div class="w-full lg:w-1/4 h-fit pb-4">
                  <div class="bg-white rounded-lg">
                      ...
                      <div id="todo" class="p-4 connectedSortable min-h-[200px]">
                      </div>
                  </div>
              </div>

              <!-- In Progress Column -->
              <div class="w-full lg:w-1/4 h-fit pb-8">
                  <div class="bg-white rounded-lg">
                      <div class="bg-primary-orange text-white pt-2 rounded-t-lg">
                          <div class="text-md primary-black font-semibold bg-white px-3 pt-1">In progress</div>
                      </div>
                      <div id="in_progress" class="p-4 connectedSortable min-h-[200px]" ondrop="taskManager.drop(event)"
              ...
              <div class="w-full lg:w-1/4 h-fit">
                  <div class="bg-white rounded-lg">
                      <div class="bg-sky-blue text-white pt-2 rounded-t-lg">
                          <div class="text-md primary-black font-semibold bg-white px-3 pt-1">Review</div>
                      </div>
                      <div id="review" class="p-4 connectedSortable min-h-[200px]" ondrop="taskManager.drop(event)"
                          ondragover="taskManager.allowDrop(event)">
                      </div>
                  </div>
              </div>
              ...
              <div class="w-full lg:w-1/4 h-fit">
                  <div class="bg-white rounded-lg">
                      ...
                      <div id="completed" class="p-4 connectedSortable min-h-[200px]" ondrop="taskManager.drop(event)"
                          ondragover="taskManager.allowDrop(event)">
                      </div>
                  </div>
              </div>
...
@if ($canTransfer)
<div id="modalTransfer" class="fixed inset-0 z-[999] flex items-center justify-center bg-gray-900 bg-opacity-50 hidden">
  <!-- Modal Content -->
  ...
  <div class="bg-white rounded-lg shadow-lg w-full max-w-xl mx-4 h-fit pb-10">
      ...
      <div class="flex justify-between items-center border-b px-4">
          ...
          <div class="flex mr-1 space-x-2">
              ...
              @endif
              <button id="closeModalShowBtn" class="text-gray-500 hover:text-gray-700 text-[32px]" onclick="taskManager.closeModalShow('modalShow')">×</button>
          </div>
      </div>
...
<div id="modalEdit" class="fixed inset-0 z-[999] flex items-center justify-center bg-gray-900 bg-opacity-50 hidden">
  ...
  <div class="bg-white rounded-lg shadow-lg w-full max-w-md mx-4 h-fit pb-10">
      ...
      <div class="flex justify-between items-center border-b px-4">
          ...
          <button class="text-gray-500 hover:text-gray-700 text-[32px]" onclick="taskManager.closeModalShow('modalEdit')">×</button>
      </div>
