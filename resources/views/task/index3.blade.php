<x-layouts.layout :title="$title" :active="$active">
    <main class="p-2">
        <div class="flex items-center justify-between w-full pb-2 mb-3">
            <div class="flex w-[40%] sm:gap-x-3 xs:gap-x-1.5">
                <div class="w-fit relative z-20">
                    <button id="dropdown-button-projects"
                        class="inline-flex items-center w-full px-2 sm:py-1 xs:py-1.5 sm:text-sm xs:text-[12px] font-medium text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm z-20"
                        onclick="taskManager.toggleDropdown('projects-dropdown', 'projects-icon', event)">
                        <span class="mr-auto" id="project-value">Projects</span>
                        <svg id="projects-icon" xmlns="http://www.w3.org/2000/svg"
                            class="sm:w-5 sm:h-5 xs:w-4 xs:h-4 ml-2 transform transition-transform duration-500"
                ...
                <div class="w-fit">
                    <input type="date" name="date-filter" id="date-filter"
                        class="w-full px-2 sm:py-[1.5px] xs:py-1.5 rounded-md border sm:text-[100%] xs:text-[12px]"
                        onchange="taskManager.fetchTasks(this.value)" value="{{ date('Y-m-d') }}">
                </div>
            </div>
            @if ($errors->has('error'))
                <div class="flex justify-center items-center mx-4">
                    <div class="flex flex-row justify-center items-center bg-red-100 rounded-lg p-2 sm:text-xs xs:text-[11px] text-red-700 w-fit whitespace-nowrap"
                        role="alert">
                ...
                @if (auth()->user()->role === 'admin' || auth()->user()->employee->role->name === 'Project Director')
                    ...
                    <div class="sm:w-1/3">
                        <a href="{{ route('tasks.allTask') }}"
                            class="bg-primary-black flex items-center justify-center w-full text-xs text-white px-4 sm:py-2 xs:py-1.5 rounded-md hover:bg-zinc-700 whitespace-nowrap">
                            See All Task
                        </a>
                    </div>
                    {{-- Mobile Layout --}}
                    <div
                        class="sm:hidden xs:flex absolute bottom-[9vh] right-0 left-auto m-0 bg-white w-full justify-center gap-4 p-5 rounded-t-xl shadow-[0px_0px_5px_5px_rgba(0,0,0,0.02)]">
                        <div class="w-1/2">
                            <button id="openModalTransferBtnMobile"
                                class="bg-primary-black flex items-center justify-center w-full text-xs text-white px-4 py-2 rounded-md hover:bg-zinc-700 whitespace-nowrap">
                                <svg class="size-3 text-white mr-1" viewBox="0 0 18 18" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path
                        ...
                        {{-- Mobile Layout --}}
                        <div class="w-1/2 block sm:hidden">
                            <button id="openModalCreateBtnMobile"
                                class="bg-primary-black flex items-center justify-center w-full text-xs text-white px-4 py-2 rounded-md hover:bg-zinc-700 whitespace-nowrap">
                                <svg class="size-4 text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
                                    width="24" height="24" fill="none" viewBox="0 0 24 24">
                                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"
        ...
        <div class="container mx-auto max-w-full">
            <div class="sm:flex xs:grid xs:grid-cols-2 sm:space-x-4 xs:gap-3 sm:gap-0 w-full" id="kanban-board">
                <!-- To Do Column -->
                <div class="sm:w-1/4 xs:w-full h-fit pb-4">
                    <div class="bg-white rounded-lg sm:shadow-none xs:shadow-[0px_0px_0px_2px_rgba(0,0,0,0.03)]">
                        <div class="bg-primary-red text-white pt-2 rounded-t-lg">
                            <div
                                class="sm:text-[16px] xs:text-[16px] primary-black font-semibold bg-white px-3 sm:pt-2 xs:pt-1">
                                To do</div>
                        </div>
                ...
                <!-- In Progress Column -->
                <div class="sm:w-1/4 xs:w-full h-fit pb-8">
                    <div class="bg-white rounded-lg sm:shadow-none xs:shadow-[0px_0px_0px_2px_rgba(0,0,0,0.03)]">
                ...
                <!-- Review Column -->
                <div class="sm:w-1/4 xs:w-full h-fit">
                    <div class="bg-white rounded-lg sm:shadow-none xs:shadow-[0px_0px_0px_2px_rgba(0,0,0,0.03)]">
        ...
        <div id="no_tasks" class="hidden sm:mt-0 xs:mt-3">
            <div class="flex items-center justify-center w-full h-full">
                <div class="flex justify-center items-center space-x-1 text-sm text-gray-700">
                    <div class="sm:text-xl xs:text-[18px]">Tidak ada tasks untuk tanggal ini</div>
                </div>
            </div>
        </div>
    </main>
...
<div id="modalCreate" class="fixed inset-0 z-[999] flex items-center justify-center bg-gray-900 bg-opacity-50 hidden">
    ...
    <div class="bg-white rounded-lg shadow-lg w-full max-w-md h-[340px]">
        ...
        <div class="flex justify-between items-center border-b px-4">
            <h2 class="sm:text-[16px] xs:text-[14px] font-semibold">Create Task</h2>
        ...
        <form action="{{ route('tasks.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="py-4 px-8 space-y-3">
                <!-- Task Name -->
                <div class="space-y-1">
                    <label class="block sm:text-sm xs:text-[10px] text-gray-700">Task</label>
                    <input type="text" placeholder="Task name..." name="name"
                        class="w-full bg-primary-white border border-primary-white px-3 py-1 sm:text-sm xs:text-[12px] rounded focus:outline-none">
                </div>

                <!-- Project -->
                <div class="space-y-1 relative">
                    <label class="block sm:text-sm xs:text-[10px] text-gray-700">Project</label>
                    <button id="dropdown-create-task"
                        ...
                        <span class="mr-auto sm:text-sm xs:text-[10px]" id="create-project-value">Select
                            Projects</span>
                        <svg id="create-task-icon" class="w-5 h-5 ml-2 transform transition-transform duration-500"
                            viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                            <path fill-rule="evenodd"
                                d="M6.293 9.293a1 1 0 011.414 0L10 11.586l2.293-2.293a1 1 0 111.414 1.414l-3 3a1 1 0 01-1.414 0l-3-3a1 1 0 010-1.414z"
                                clip-rule="evenodd" />
                    ...
                    <ul id="create-task-dropdown"
                        ...
                        @foreach ($projects as $project)
                            <li class="block px-4 py-2 text-black hover:bg-[#C3C3C3] cursor-pointer rounded-md sm:text-sm xs:text-[12px]"
                ...
                <!-- Task Level -->
                <div>
                    <div class="flex items-center sm:justify-normal xs:justify-between">
                        <label class="block sm:text-sm xs:text-[10px] text-gray-700 mr-3">Task Level</label>
                        <div class="flex space-x-4 items-center">
                            <label class="flex items-center sm:text-sm xs:text-[12px]">
                    ...
                    <p
                        class="sm:text-xs xs:text-[10px] bg-primary-white py-1 px-3 rounded text-slate-600 mt-2 sm:space-x-6 xs:space-x-10 xs:text-center sm:text-left">
            ...
            <div class="flex w-full px-8 pt-3">
                <button id="submitBtn" type="submit"
                    class="bg-primary-black w-full sm:text-sm xs:text-[12px] text-white px-4 py-2 rounded hover:bg-zinc-700">
                    Submit
                </button>
            </div>
        </form>
...
<div id="modalTransfer"
    ...
    <div class="bg-white rounded-lg shadow-lg w-full max-w-md h-fit pb-10">
        <!-- Modal Header -->
        <div class="flex justify-between items-center border-b px-4">
            <h2 class="sm:text-[16px] xs:text-[14px] font-semibold">Transfer Task</h2>
        ...
        <form action="{{ route('tasks.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="py-4 px-8 space-y-3">
                <!-- Task Name -->
                <div class="space-y-1">
                    <label class="block sm:text-sm xs:text-[10px] text-gray-700" for="name">Task</label>
                ...
                <!-- Project -->
                <div class="space-y-1 relative">
                    <label class="block sm:text-sm xs:text-[10px] text-gray-700" for="project_id">Project</label>
                    <button id="dropdown-transfer-task"
                        class="inline-flex items-center w-full px-3 py-1 sm:text-sm xs:text-[12px] bg-primary-white border border-primary-white rounded-md shadow-sm"
                        onclick="taskManager.toggleDropdown('trasfer-task-dropdown', 'trasfer-task-icon', event)">
                        <span class="mr-auto sm:text-sm xs:text-[12px]" id="trasfer-task-value">Select Projects</span>
                ...
                @if (auth()->user()->role === 'admin' || auth()->user()->employee->role->name === 'Project Director')
                    <div class="space-y-1 relative">
                        <label class="block sm:text-sm xs:text-[10px] text-gray-700"
                            for="assigned_project_employee_id">Send to</label>
                        <button id="dropdown-transfer-employee"
                            class="inline-flex items-center w-full px-3 py-1 sm:text-sm xs:text-[12px] bg-primary-white border rounded-md shadow-sm"
                            onclick="taskManager.toggleDropdown('trasfer-employee-dropdown', 'trasfer-employee-icon', event)"
                            disabled>
                            <span class="mr-auto sm:text-sm xs:text-[12px]" id="trasfer-employee-value">Select
                                employee</span>
                        ...
                        <ul id="trasfer-employee-dropdown"
                            class="w-full absolute right-0 border mt-2 rounded-md shadow-lg bg-primary-white p-1 space-y-1 transform opacity-0 scale-95 -translate-y-2 hidden transition-all duration-300 ease-out origin-top z-50 sm:text-sm xs:text-[12px]">
                        </ul>
...
{{-- modal edit task --}}
<div id="modalShow" class="fixed inset-0 z-[999] flex items-center justify-center bg-gray-900 bg-opacity-50 hidden">
    <!-- Modal Content -->
    <div class="bg-white rounded-lg shadow-lg w-full max-w-xl h-fit pb-10">
        ...
        <div class="flex justify-between items-center border-b px-4">
            ...
            <div class="flex mr-1 space-x-2">
                ...
                <button id="closeModalTransferBtn" class="text-gray-500 hover:text-gray-700 text-[32px]"
...
<div id="modalEdit" class="fixed inset-0 z-[999] flex items-center justify-center bg-gray-900 bg-opacity-50 hidden">
    <!-- Modal Content -->
    <div class="bg-white rounded-lg shadow-lg w-full max-w-md h-fit pb-10">
        <!-- Modal Header -->
        <div class="flex justify-between items-center border-b px-4">
            ...
            <button class="text-gray-500 hover:text-gray-700 text-[32px]"
                onclick="taskManager.closeModalShow('modalEdit')">&times;</button>
        </div>
...
<script>
    window.employeeId = @js(auth()->user()->employee?->id ?? null);
</script>
