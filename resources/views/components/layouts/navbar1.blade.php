<nav class="py-4 flex justify-between items-center bg-white sticky top-0 z-50 border-b">
    <div class="flex justify-between items-center w-full">
            ...
            <div class="flex items-center relative">
                <div class="w-full">
                    <input type="text" id="search" name="search" placeholder="Search project"
                        ...
                        required>
                    <svg class="absolute size-6 right-3 top-1/2 -translate-y-1/2 primary-gray" viewBox="0 0 41 41"
        ...
        <div class="flex gap-x-5 relative">
            <div class="flex items-center">
                ...
                <div class="hidden md:flex md:flex-col md:items-end mr-3">
                ...
                <div class="group flex flex-col items-center">
                    @if (auth()->user()->employee)
                        <a class="group w-12 h-12 relative overflow-hidden bg-secondary-white border rounded-full hover:bg-blue-300 transition-colors duration-300"
                            href={{ route('users.profile') }} :active="$active">
                        @else
                            <div
                                class="group relative overflow-hidden bg-secondary-white border rounded-full hover:bg-blue-300 transition-colors duration-300">
                            </div>
                    @endif
            ...
            <div class="flex flex-col items-center justify-center group mr-4 ml-3">
                <form method="POST" action="{{ route('logout') }}" id="logout-form">
                    @csrf
                    <button type="button" id="logout-btn"
                        class="bg-secondary-white border p-3 text-sm flex items-center justify-center rounded-full hover:bg-blue-300 transition-colors duration-300">
                    ...
                    <div
                        class="group-hover:block hidden text-sm text-white mt-[10px] right-0 ml-5 px-5 rounded-md absolute bg-sky-blue primary-white">
                        Logout
                    </div>
                </form>
            </div>
