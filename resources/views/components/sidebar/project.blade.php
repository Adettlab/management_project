...
<div class="group flex items-center xs:relative">
    <a class="sm:bg-[#F5F5F5] p-2 flex items-center justify-center rounded-full cursor-pointer sm:hover:bg-[#6FAEC9] {{ $active === 'projects' ? 'sm:bg-sky-blue xs:bg-transparent' : 'bg-white' }}"
    ...
    <div
        class="sm:hidden xs:block sm:group-hover:block sm:text-sm xs:text-[9px] sm:ml-14 text-center w-32 py-[2px] rounded-md absolute xs:-bottom-3 xs:-left-11 sm:left-auto sm:bottom-auto sm:bg-sky-blue sm:bg-white z-[999] group-hover:z-[999] {{ $active === 'projects' ? 'xs:text-[#6FAEC9] sm:text-white' : 'xs:text-black sm:text-white' }}">
        Project
    </div>
</div>
