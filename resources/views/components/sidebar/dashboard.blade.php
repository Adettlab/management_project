...
<div class="group w-full mb-3 flex items-center z-50 cursor-pointer"
    :class="{
        ...
        'hover:bg-blue-400 hover:text-white rounded-full {{ $active === 'dashboard' ? 'bg-blue-300' : '' }}': sidebarExpanded
    }"
    onclick="window.location.href='{{ route('dashboard') }}'">
    <a href="{{ route('dashboard') }}"
        class="p-2 flex items-center justify-center z-50 hover:z-50 rounded-full cursor-pointer group-hover:bg-blue-400"
        :class="{
            'group-hover:text-white rounded-full {{ $active === 'dashboard' ? 'bg-blue-300' : 'bg-white' }}': sidebarExpanded,
            'rounded-full {{ $active === 'dashboard' ? 'bg-blue-300' : 'bg-gray-200' }}': !sidebarExpanded
        }">
        <svg class="size-6" viewBox="0 0 35 35" xmlns="http://www.w3.org/2000/svg">
            <path class="{{ $active === 'dashboard' ? 'fill-white' : 'fill-gray-600' }} group-hover:fill-white"
    ...
    <a href="{{ route('dashboard') }}" x-show="sidebarExpanded" class="ml-1 pr-2 text-sm font-medium"
        :class="{
            'group-hover:text-white {{ $active === 'dashboard' ? 'text-white' : '' }}': sidebarExpanded,
            '': !sidebarExpanded
        }">
        Dashboard
    </a>
