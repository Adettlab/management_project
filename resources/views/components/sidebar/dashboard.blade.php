@props(['active'])
<div class="group sm:flex items-center z-50 xs:relative sm:w-full">
    <a href="{{ route('dashboard') }}"
        class="sm:bg-[#F5F5F5] sm:w-full p-2 flex items-center justify-center sm:justify-start rounded-full z-50 hover:z-50 cursor-pointer group sm:hover:bg-[#6FAEC9] sm:rounded-md sm:hover:text-white {{ $active === 'dashboard' ? 'sm:bg-sky-blue xs:bg-transparent' : 'bg-white' }}">
        <svg class="size-6" viewBox="0 0 35 35" xmlns="http://www.w3.org/2000/svg">
            <path
                class="{{ $active === 'dashboard' ? 'sm:fill-[#FFFFFF] xs:fill-[#6FAEC9]' : 'fill-[#616161]' }} sm:group-hover:fill-[#FFFFFF]"
        ...
        <span class="hidden sm:block sm:ml-1 sm:text-sm {{ $active === 'dashboard' ? 'sm:text-white' : '' }}">
            Dashboard
        </span>
    </a>
    <div class="sm:hidden xs:block sm:text-sm xs:text-[9px] sm:ml-14 text-center w-32 py-[2px] rounded-md absolute xs:-bottom-3 xs:-left-11 sm:left-auto sm:bottom-auto sm:bg-sky-blue sm:bg-white z-[999] group-hover:z-[999] {{ $active === 'dashboard' ? 'xs:text-[#6FAEC9] sm:text-white' : 'xs:text-black sm:text-white' }}"
        style="z-index: 999;">
        Dashboard
    </div>
</div>
