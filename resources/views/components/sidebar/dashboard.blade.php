...
<div class="group flex items-center z-50 cursor-pointer {{ $active === 'dashboard' ? 'bg-sky-blue' : 'bg-white' }}"
    :class="{
    ...
    <a href="{{ route('dashboard') }}"
        class="bg-secondary-white p-2 flex items-center justify-center z-50 hover:z-50 rounded-full cursor-pointer group-hover:bg-[#6FAEC9] {{ $active === 'dashboard' ? 'bg-sky-blue' : 'bg-white' }}"
        ...
        <svg class="size-6" viewBox="0 0 35 35" xmlns="http://www.w3.org/2000/svg">
            <path class="{{ $active === 'dashboard' ? 'fill-[#FFFFFF]' : 'fill-[#616161]' }} group-hover:fill-[#FFFFFF]"
    ...
    <a href="{{ route('dashboard') }}" x-show="sidebarExpanded"
        class="ml-1 pr-2 text-gray-700 text-sm font-medium {{ $active === 'dashboard' ? 'text-white' : '' }}"
        :class="{ 'group-hover:text-white': sidebarExpanded, '': !sidebarExpanded }">
        Dashboard
    </a>
