...
<div class="group w-full mb-3 flex items-center z-50 cursor-pointer"
    :class="{
        ...
        'hover:bg-blue-400 hover:text-white rounded-full {{ $active === 'admin' ? 'bg-blue-300' : '' }}': sidebarExpanded
    }"
    ...
    <a href="{{ route('admin.index') }}"
        ...
        :class="{
            'group-hover:text-white rounded-full {{ $active === 'admin' ? 'bg-blue-300' : 'bg-white' }}': sidebarExpanded,
            'rounded-full {{ $active === 'admin' ? 'bg-blue-300' : 'bg-gray-200' }}': !sidebarExpanded
        }">
    ...
    <a href="{{ route('admin.index') }}" x-show="sidebarExpanded" class="ml-1 pr-2text-sm font-medium"
        :class="{
            'group-hover:text-white {{ $active === 'admin' ? 'text-white' : '' }}': sidebarExpanded,
            '': !sidebarExpanded
        }">
        Admin
    </a>
