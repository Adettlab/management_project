...
<div class="group w-full mb-3 flex items-center z-50 cursor-pointer hover:bg-blue-400 hover:text-white rounded-full {{ $active === 'admin' ? 'bg-blue-300' : '' }}"
    onclick="window.location.href='{{ route('admin.index') }}'">
    <a href="{{ route('admin.index') }}"
        class="p-2 flex items-center justify-center z-50 hover:z-50 rounded-full cursor-pointer group-hover:bg-blue-400 group-hover:text-white rounded-full {{ $active === 'admin' ? 'bg-blue-300' : 'bg-white' }}">
        <svg class="size-6" viewBox="0 0 35 35" xmlns="http://www.w3.org/2000/svg">
            <path class="{{ $active === 'admin' ? 'fill-white' : 'fill-gray-600' }} group-hover:fill-white"
    ...
    <!-- Label -->
    <a href="{{ route('admin.index') }}"
        class="ml-1 pr-2 text-sm font-medium group-hover:text-white {{ $active === 'admin' ? 'text-white' : '' }}">
        Admin
    </a>
</div>
