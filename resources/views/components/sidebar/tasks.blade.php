...
<div class="group flex items-center xs:relative">
    <a class="sm:bg-[#F5F5F5] p-2 flex items-center justify-center rounded-full cursor-pointer sm:hover:bg-[#6FAEC9] {{ $active === 'tasks' ? 'sm:bg-sky-blue xs:bg-transparent' : 'bg-white' }}"
    ...
    <div
        class="sm:hidden xs:block sm:group-hover:block sm:text-sm xs:text-[9px] sm:text-white xs:text-black sm:ml-14 text-center w-32 py-[2px] rounded-md absolute xs:-bottom-3 sm:bottom-auto xs:-left-11 sm:left-auto sm:bg-sky-blue sm:bg-white group-hover:z-50 {{ $active === 'tasks' ? 'xs:text-[#6FAEC9] sm:text-white' : 'xs:text-black sm:text-white' }}">
        Tasks
    </div>
</div>
