...
<div class="group flex items-center xs:relative sm:w-full">
    <a class="sm:bg-[#F5F5F5] sm:w-full p-2 flex items-center sm:justify-start justify-center rounded-full z-50 hover:z-50 cursor-pointer group sm:hover:bg-[#6FAEC9] sm:rounded-md sm:hover:text-white {{ $active === 'administration' ? 'sm:bg-sky-blue xs:bg-transparent' : 'bg-white' }}"
        ...
        <span class="hidden sm:block sm:ml-1 sm:text-sm {{ $active === 'administration' ? 'sm:text-white' : '' }}">
            Administration
        </span>
    </a>
    <div
        class="sm:hidden xs:block sm:text-sm xs:text-[9px] sm:ml-14 text-center w-32 py-[2px] rounded-md absolute xs:-bottom-3 xs:-left-11 sm:left-auto sm:bottom-auto sm:bg-sky-blue sm:bg-white z-[999] group-hover:z-[999] {{ $active === 'administration' ? 'xs:text-[#6FAEC9] sm:text-white' : 'xs:text-black sm:text-white' }}">
        Administration
    </div>
</div>
