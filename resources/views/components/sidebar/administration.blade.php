...
<div class="group w-full mb-3 flex items-center z-50 cursor-pointer hover:bg-blue-400 hover:text-white rounded-full {{ $active === 'administration' ? 'bg-blue-300' : '' }}"
    onclick="window.location.href='{{ route('administration.index') }}'">
    <a href="{{ route('administration.index') }}"
        class="p-2 flex items-center justify-center z-50 hover:z-50 rounded-full cursor-pointer group-hover:bg-blue-400 group-hover:text-white rounded-full {{ $active === 'administration' ? 'bg-blue-300' : 'bg-white' }}">
    ...
    <!-- Label -->
    <a href="{{ route('administration.index') }}"
        class="ml-1 pr-2 text-sm font-medium group-hover:text-white {{ $active === 'administration' ? 'text-white' : '' }}">
        Administration
    </a>
</div>
